...
# ================= 配置区 =================
DATA_DIR="code/data" 
RESULT_DIR="experiment_results_small"
        ...
        for run in {1}; do
            ...
            if [ "$algo" == "giraph" ]; then
                hadoop jar $GIRAPH_JAR \
                    ...
                    -w 1 \
                    -ca giraph.SplitMasterWorker=false,giraph.maxNumberOfSupersteps=500 > $LOG 2>&1
                
                # 🔴 关键点：任务结束后，立即尝试用 yarn logs 强制抓取
                yarn logs -applicationId $(grep -o "application_[0-9_]*" $LOG | head -1) > $RESULT_DIR/logs/${JOB_ID}_full.log 2>&1
            else
